...
class FlanT5PPOFineTuner(pl.LightningModule):
    ...
    def calculate_rewards(self, generated_texts, edited_endings, original_endings):
        """
        Computes normalized reward signal.
        reward = sim(edited) + (sim(edited) - sim(original))
        """
        def clean(t):
            return t.replace(self.tokenizer.eos_token, '').strip() if isinstance(t, str) else str(t)

        gen_cleaned = [clean(t) for t in generated_texts]
        edit_cleaned = [clean(t) for t in edited_endings]
        orig_cleaned = [clean(t) for t in original_endings]

        sim_edited = self.metrics_evaluator.calculate_score(gen_cleaned, edit_cleaned).detach()
        sim_original = self.metrics_evaluator.calculate_score(gen_cleaned, orig_cleaned).detach()

        # Raw reward formula
        if CONFIG["ppo_experiment"] == "delta_m1":
            rewards = sim_edited + (sim_edited - sim_original)
        else:
            rewards = sim_edited

        # Normalize and scale rewards
        rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-8)
        rewards = rewards * 0.5  # Optional: scale down

        # Logging for debug
        self.log_dict({
            'train/norm_sim_edited': sim_edited.mean(),
            'train/norm_sim_original': sim_original.mean(),
            'train/norm_rewards': rewards.mean()
        }, prog_bar=True)

        # Debug invalid values
        if torch.isnan(rewards).any() or torch.isinf(rewards).any():
            logger.error(f"[REWARD DEBUG] Detected invalid reward values:\n"
                        f"- NaNs: {(torch.isnan(rewards)).sum().item()}\n"
                        f"- Infs: {(torch.isinf(rewards)).sum().item()}\n"
                        f"- Sample: {rewards[:5]}")
    ...
    def _load_checkpoint_safely(self, checkpoint_path):
        """
        Safely load a PPO checkpoint, logging exactly what was applied.
        Falls back to fresh init on failure.
        """
        try:
            # 1) Load the raw checkpoint
            checkpoint = torch.load(checkpoint_path, map_location='cpu')
            # 2) Extract state_dict (handles Lightning or plain)
            state_dict = checkpoint.get('state_dict', checkpoint)

            # 3) Filter out any attention weights we don't want to overwrite
            filtered_state_dict = {
                k: v for k, v in state_dict.items()
                if not any(x in k for x in ['attention', 'attn'])
            }

            # 4) Load into our model/value_head; capture missing/unexpected keys
            load_res = self.load_state_dict(filtered_state_dict, strict=False)
            ...
            # 6) On any error, log and reinitialize the value head
            logger.error(f" Checkpoint loading failed ({checkpoint_path}): {e}")
            logger.info("Initializing model/value head from scratch")
            self._initialize_value_head()
   
    def _initialize_value_head(self):
        """Robust initialization of value head"""
        try:
            with torch.no_grad():
                ...
                logger.info("Value head initialized with small normal weights")
        except Exception as e:
            logger.error(f"Value head init failed: {str(e)}")
