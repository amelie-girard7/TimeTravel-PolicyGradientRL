...
CONFIG = {
    ...
    "num_workers":    int(os.getenv('NUM_WORKERS', 3)),  # DataLoader worker threads
    "learning_rate":  float(os.getenv('LEARNING_RATE', 5e-6)),  # LR for the policy (T5)
    "value_lr":       float(os.getenv('VALUE_LR', 5e-6)),      # LR for the value head
    ...
    "top_p":              0.9,     # Nucleus (top‑p) sampling filter


    # === PPO Core Parameters ===
    "ppo_experiment":        "delta_m1",     # Reward‑shaping variant
    "training_epochs":       int(os.getenv("TRAINING_EPOCHS", 15)),  # Epochs over entire dataset
    "ppo_epochs":            15,       # Inner‑loop gradient passes per PPO update
    "ppo_clip_epsilon":      0.1,     # Clip range ε for policy ratio
    "entropy_coef":          0.01,    # Weight on entropy bonus
    "value_coef":            0.5,     # Weight on value loss term
    "max_trajectory_length": 512,     # Token budget before triggering PPO update
    ...
    "gradient_clip_val":  1.0,        # Clip gradients by global norm
    ...
    "reward_margin":         0.2,     # Contrastive reward margin
