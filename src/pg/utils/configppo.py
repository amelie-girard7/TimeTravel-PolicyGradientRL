...
# TIMETRAVEL_ROOT can override the default root.
...
CONFIG = {
    ...
    "dataset_type": "TimeTravel",  # Options: "TimeTravel", "ART", etc.
    ...
    "learning_rate": float(os.getenv('LEARNING_RATE', 1e-5)),  # Learning rate
    ...
    "ppo_experiment": "delta_m1",     # Reward shaping method
    "ppo_epochs": 4,                  # PPO epochs per trajectory batch
    "ppo_clip_epsilon": 0.2,          # Clipping epsilon for PPO ratio
    "entropy_coef": 0.01,             # Entropy bonus coefficient
    "value_coef": 0.5,                # Value loss coefficient
    "max_trajectory_length": 1024,    # Samples before a PPO update
    "gamma": 0.99,                    # Discount factor
    "lambda": 0.95,                   # GAE lambda (bias/variance trade-off)
    "cache_teacher_states": True,     # Option to cache teacher states

    # === PPO Initialization Mode ===
    "init_value_head": "from_mle",    # Options: "random" or "from_mle"

    # === Reward Signal Metric ===
    "reward_metric": "bart",          # Options: "bart", "rouge", "bert", "bleu"

    # === Optional: Objective Clipping ===
    "objective_clipping": False,      # Reward clipping

    # === Resume PPO from a Pretrained Checkpoint ===
    "ppo_from_checkpoint": True,      # Load weights from checkpoint if True
    ...
    # Resume training from checkpoint flag.
