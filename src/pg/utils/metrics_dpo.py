# /data/agirard/Projects/TimeTravel-PolicyGradientRL/src/pg/utils/metrics_dpo.py
import logging
...
class MetricsEvaluator:
    ...
    def calculate_score(self, generated_texts, references):
        ...
        # Handle empty generated texts explicitly for ROUGE
        # if score_metric == "rouge":
        #     scores = []
        #     for gt, ref in zip(generated_texts, references):
        #         if not gt.strip():  # Check if generated text is empty or only whitespace
        #             scores.append(-1.0)  # Penalize empty text
        #         else:
        #             try:
        #                 rouge_score = self.rouge.get_scores(gt, ref, avg=False)
        #                 scores.append(rouge_score[0]['rouge-l']['f'])  # Use ROUGE-L F1 score
        #             except Exception as e:
        #                 print(f"Error calculating ROUGE score: {e}")
        #                 scores.append(-1.0)  # Fallback to penalizing invalid ROUGE calculations
    ...
    def calculate_and_log_bert_similarity(self, all_generated_texts, all_edited_endings, all_counterfactuals,
                                          all_initials, all_premises, all_original_endings, logger):
        """
